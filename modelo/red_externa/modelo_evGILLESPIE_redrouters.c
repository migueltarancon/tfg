...
int** readRed(char *filered, int N);
int size_poblacion(state_t *state, state_t poblacion, int N);

main(int argc, char *argv[])
{
    int N=10000, **red; //poner numero de nodos en la red
    double seed_b=0.05, seed_w=0.05;    //semillas de infectados B y W
    ...
    char filered[100];
    sprintf(filered, "red.csv");    //nombre del archivo donde esta la red
    red=readRed(filered, N);
...
{
    char filename[100];
    sprintf(filename, "modelo_gill.txt");   //donde se quieren guardar los resultados
...
int** readRed(char *filered, int N)
{
    ...
    FILE *f=fopen(filered, "r");
    while(!feof(f)){
        fscanf(f, "%d,%d,%s\n", &ori, &fin, dummy);
        red[ori][fin]=red[fin][ori]=1;
    }
